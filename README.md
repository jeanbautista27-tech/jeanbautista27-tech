<div align="center">

# 👋 Hola, soy Jenner Bautista

### 💻 Estudiante de Ingenieria de Sistemas | Backend Developer | Java & Spring | Data & AI

</div>

Soy estudiante de **Ingeniería de Sistemas en la UNSCH**, orientado al
desarrollo de software y especialmente al desarrollo Backend con
**Java y Spring Framework**.

Me interesa construir soluciones de software funcionales y escalables,
aplicando buenas prácticas de desarrollo, metodologías de ingeniería
de software y tecnologías modernas.

Actualmente estoy fortaleciendo mis conocimientos en **Inteligencia
Artificial, Machine Learning, Data Engineering, DevOps, MLOps, Redes y
Telecomunicaciones**, mediante proyectos prácticos y aprendizaje
continuo.

---

## 🚀 Sobre mí

- 🎓 Estudiante de **Ingeniería de Sistemas – UNSCH**
- 💻 Desarrollo de aplicaciones y sistemas **Backend**
- ☕ Desarrollo con **Java y Spring Framework**
- 🔗 Desarrollo e integración de **APIs REST**
- 🗄️ Bases de datos y gestión de información
- ⚙️ Aprendiendo **DevOps y MLOps**
- 🤖 Aprendiendo **Inteligencia Artificial y Machine Learning**
- 📊 Interés en **Data Engineering y análisis de datos**
- 🌐 Fortaleciendo conocimientos en **Redes y Telecomunicaciones**
- 🐳 Trabajo con **Docker, Git y GitHub**

---

## 🛠️ Tecnologías y herramientas

### 💻 Backend
Java · Spring Boot · Python · Node.js · JavaScript · REST APIs

### 🗄️ Bases de Datos
PostgreSQL · MySQL · SQL Server

### ⚙️ DevOps & Sistemas
Docker · Git · GitHub · Linux · CI/CD

### 🤖 Data & AI
Python · NumPy · Pandas · Scikit-learn · Machine Learning ·
Inteligencia Artificial

### 🎨 Frontend
JavaScript · React · HTML · CSS

### 🌐 Redes
Redes de Computadoras · Telecomunicaciones · Linux

---

## 📌 Proyectos Destacados

### 🏪 Sistema de Gestión para Ferretería

Sistema de gestión desarrollado como **trabajo final universitario**,
aplicando la metodología **SSD (Software Systems Development)**.

Actualmente se encuentra **desplegado y funcional**.

### 🚗 Sistema de Gestión de Parqueo

Sistema de gestión para el control y administración de un parqueo,
desarrollado como proyecto práctico.

Actualmente se encuentra **funcional en entorno local**.

---

## 📚 Actualmente aprendiendo

- 🤖 **Inteligencia Artificial**
- 🧠 **Machine Learning**
- ⚙️ **MLOps**
- 🔧 **DevOps & CI/CD**
- 📊 **Data Engineering**
- 🌐 **Redes y Telecomunicaciones**
- ☁️ **Cloud Computing**
- 🏗️ **Arquitectura de Software**

---

## 🧰 Lenguajes y herramientas

<p align="left">

<a href="https://www.java.com/">
<img src="https://skillicons.dev/icons?i=java" width="50" />
</a>

<a href="https://spring.io/">
<img src="https://skillicons.dev/icons?i=spring" width="50" />
</a>

<a href="https://www.python.org/">
<img src="https://skillicons.dev/icons?i=python" width="50" />
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript">
<img src="https://skillicons.dev/icons?i=javascript" width="50" />
</a>

<a href="https://react.dev/">
<img src="https://skillicons.dev/icons?i=react" width="50" />
</a>

<a href="https://nodejs.org/">
<img src="https://skillicons.dev/icons?i=nodejs" width="50" />
</a>

<a href="https://www.docker.com/">
<img src="https://skillicons.dev/icons?i=docker" width="50" />
</a>

<a href="https://git-scm.com/">
<img src="https://skillicons.dev/icons?i=git" width="50" />
</a>

<a href="https://github.com/">
<img src="https://skillicons.dev/icons?i=github" width="50" />
</a>

<a href="https://www.linux.org/">
<img src="https://skillicons.dev/icons?i=linux" width="50" />
</a>

<a href="https://www.postgresql.org/">
<img src="https://skillicons.dev/icons?i=postgresql" width="50" />
</a>

<a href="https://www.mysql.com/">
<img src="https://skillicons.dev/icons?i=mysql" width="50" />
</a>

</p>

---

## 📫 Contacto

📧 **jean.bautista.27@unsch.edu.pe**

💼 **LinkedIn: https://www.linkedin.com/in/jean-jenner-bautista-figueroa-346534438/ ** 

---

⭐ *Construyendo, aprendiendo y mejorando un proyecto a la vez.*
